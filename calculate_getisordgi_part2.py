...
for i in range(len(t0)):
    ...
    final = final.append(get_ordg_hotspots_from_shp2(tmp,w_surface,['rf_no2','no2_cmaq_crop']))
...
p = ['rf_no2','no2_cmaq_crop','getisord_rf_no2','getisord_no2_cmaq_crop']
vmins = [0,0,'','']
vmaxs = [20,20,'','']
...
for i in range(len(ax)):
    crop = final[final.date == t[0]].reset_index()
    crop.plot(p[i],ax=ax[i],vmin = vmins[i],vmax=vmaxs[i])

plt.tight_layout()
plt.show()

I’m available next week to discuss this collaboration opportunity, but I’m happy to work around your and Dan’s schedule to set up a call.

p = ['getisord_rf_no2','getisord_no2_cmaq_crop']
fig,axs = plt.subplots(10,2,figsize=(4,10))
for i in range(len(axs)):
    ax = axs[i]
    crop = final[final.date == t[i]].reset_index()
    ch = crop[crop[p[0]] == 'high']
    #cl = crop[crop[p[0]] == 'low']
    cns = crop[crop[p[0]] == 'ns']
    cns.plot(facecolor='gray',ax=ax[0])
    #cl.plot(facecolor='blue',ax=ax[0])
    ch.plot(facecolor='red',ax=ax[0])
    ch = crop[crop[p[1]] == 'high']
    #cl = crop[crop[p[1]] == 'low']
    cns = crop[crop[p[1]] == 'ns']
    cns.plot(facecolor='gray',ax=ax[1])
    #cl.plot(facecolor='blue',ax=ax[1])
    ch.plot(facecolor='red',ax=ax[1])
